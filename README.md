...
```
Step 1  灵感捕获      录音/文字自由倾倒 → raw_requirements.md
          │
Step 2  SDD 生成      新建 AI 对话 → spec_v1.md
          │
Step 3  压力测试      全新 AI 对话（必须隔离）→ scorecard_v1.json
          │
          ├── 发现漏洞 → Step 4
          └── 收敛（0 高 + ≤3 中）→ Step 5
          │
Step 4  反馈修正      回到 Step 2 对话修订 → spec_v2.md → 重回 Step 3
          │
Step 5  锁定执行      spec_final.md → Claude Code 无人值守执行
```

**详细说明：** `skills/planning-workflow.md`
**快速参考：** `plan/quick_reference.md`
...
1. 描述变更原因 + 影响的 Spec 章节
2. 复制 `spec_final.md` → `spec_final_v2.md`
3. 只修改必须改的章节
4. 对新版本重新执行 Step 3 压力测试
5. 满足收敛阈值后锁定新版本

**完整 CR 流程：** 参见 `plan/quick_reference.md` 中的"变更请求流程"章节。

---

## 项目复盘（Phase 0）

每个项目完成后，使用 `plan/template_00_postmortem.md` 执行复盘：
...
压力测试的退出标准：**0 高严重度 + ≤3 中严重度**

未达标时循环 Step 3-4 直到收敛。超过 5 轮未收敛，回到 Step 1 重新梳理需求。
