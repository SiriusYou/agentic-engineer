# Template 03 — 对抗性压力测试
# 用途：用分层攻击性问题（5-20 题，按项目类型组装）找出 SDD 的设计漏洞
...
- passed: true 表示设计已充分覆盖该场景，或该场景不适用于本项目（`severity: "n/a"`）；false 表示存在漏洞
- severity: "none"（通过）/ "low" / "medium" / "high" / "n/a"（不适用）
...
| 项目类型 | 问题集 | 题数 |
|---------|--------|------|
| 无状态 CLI 工具（lint、格式化、检查器） | C1-C5 | 5 |
| 有状态 CLI / 纯逻辑库 | U1-U10 | 10 |
| 功能级 MVP（无独立持久化） | U1-U10 | 10 |
| Web 应用 / API 服务 | U1-U10 + W1-W5 | 15 |
| 数据密集型系统 | U1-U10 + D1-D5 | 15 |
| 全栈 Web + 数据系统 | U1-U10 + W1-W5 + D1-D5 | 20 |

> **无状态 CLI 判定：** 无持久化状态、无网络 I/O、输入仅来自文件系统/stdin、输出仅 stdout/stderr + exit code。满足全部条件时使用 C1-C5 替代通用层。

> **功能级 MVP 升级条件：** 仅当 MVP 功能本身包含 HTTP 接口时追加 W1-W5（→15题），或本身包含持久化时追加 D1-D5（→15题）。若同时需要 Web 接口和持久化，应归类为"全栈 Web + 数据系统"走 20 题标准集（U10+W5+D5），不使用功能级 MVP 路径。

> **层级跳过 vs 单题 N/A 的区别：**
> - **层级跳过**（推荐）：如果 MVP 功能整体不涉及某层（如无持久化 → 跳过 D1-D5），直接不提问该层。scorecard 中不出现这些题号，题目总数相应减少。
> - **单题 N/A**：在适用层内，个别题目因项目特征不适用时，仍记录该题并标记 `severity: "n/a"`。
> - **主规则**：优先层级跳过，仅在"层级整体适用但个别题不适用"时使用单题 n/a。
