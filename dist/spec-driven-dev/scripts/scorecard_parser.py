...
JSON output contract (--format json):
{
    ...
    "entries": [                    # sorted by prefix order (C->U->W->D), then numeric
...
- Represents questions not applicable to the project
...
def generate_markdown(entries, version, date_str):
    ...
    lines = [
        "## Stress Test Vulnerability Report",
        f"Date: {date_str}",
        f"Spec Version: {version}",
        "",
        "| ID | Pass | Vulnerability | Severity |",
        "|----|------|---------------|----------|",
    ]
...
def generate_convergence(entries):
    ...
    if converged:
        lines.append(
            f"CONVERGED ({high_count} high + {medium_count} medium) -> Lock spec, proceed to Step 5"
        )
    else:
        lines.append(
            f"NOT CONVERGED ({high_count} high + {medium_count} medium) -> Iterate with Step 4"
        )
...
def create_parser():
    ...
    parser = argparse.ArgumentParser(
        description="Parse stress test scorecards into vulnerability reports."
    )
    parser.add_argument(
        "scorecard",
