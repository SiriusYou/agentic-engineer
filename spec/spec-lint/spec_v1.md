...
### 1.2 核心用户场景
- 作为方法论使用者，我希望在锁定 spec 前运行格式检查，从而避免遗漏必填章节
- 作为 CI pipeline，我希望自动检查 SDD 文档完整性，从而保证质量门禁
- 作为压力测试者，我希望验证模式定义章节的正反例完整性，从而提高检查器质量
...
- **输入**：Markdown 文本（字符串）
- **输出**：`Document` 数据结构（章节列表，每个含标题、层级、行号、内容行）
- **依赖**：无
- **不负责**：验证章节内容的语义正确性
...
- **输出**：`list[LintResult]`
- **匹配模式**：`^##\s+(\d+)\.\s+`
- **正例**：`## 1. 项目概述` → 匹配章节 1；`## 3. 系统架构 [Required]` → 匹配章节 3；`## 6. 错误处理策略` → 匹配章节 6
- **反例**：`### 1.1 目标` → 不匹配（三级标题）；`# 1. 项目概述` → 不匹配（一级标题）；`## A. 附录` → 不匹配（非数字编号）
...
- **输出**：`list[LintResult]`
- **匹配模式（版本）**：`^版本:\s*v\d+\.\d+`
- **正例**：`版本: v1.0` → 匹配；`版本: v2.1` → 匹配；`版本:v1.0` → 匹配（冒号后无空格）
- **反例**：`Version: v1.0` → 不匹配（英文）；`版本: 1.0` → 不匹配（缺少 v 前缀）；`版本: draft` → 不匹配（非版本号）
- **匹配模式（状态）**：`^状态:\s*(Draft|Review|Final)`
- **正例**：`状态: Draft` → 匹配；`状态: Final` → 匹配；`状态:Review` → 匹配
- **反例**：`状态: WIP` → 不匹配（无效状态值）；`Status: Draft` → 不匹配（英文）；`状态: draft` → 不匹配（小写）
- **匹配模式（日期）**：`^最后更新:\s*\d{4}-\d{2}-\d{2}`
- **正例**：`最后更新: 2026-03-03` → 匹配；`最后更新:2026-01-15` → 匹配；`最后更新: 2025-12-31` → 匹配
- **反例**：`最后更新: 03/03/2026` → 不匹配（非 ISO 格式）；`Last updated: 2026-03-03` → 不匹配（英文）；`最后更新: TBD` → 不匹配

#### TBDMarkerChecker
- **职责**：检测文档中残留的待定标记
- **输入**：`Document` 全文
- **输出**：`list[LintResult]`
- **匹配模式**：`(?i)\b(TBD|TODO|待定|待补充|未决定)\b`
- **正例**：`数据库选型: TBD` → 匹配；`认证方式待定` → 匹配；`TODO: 补充错误码` → 匹配
- **反例**：`使用 TBDMarkerChecker 检查` → 不匹配（代码/类名中的 TBD）— **已知边界：代码块内不检查**；`completed` → 不匹配；`table` → 不匹配
- **已知边界**：代码块（```...```）内的内容不检查

#### ExitCodeTableChecker
- **职责**：当检测到 CLI 接口变体（4.B）时，验证 exit code 表存在且包含 0/1/2
- **输入**：`Document` 的章节 4 内容
- **输出**：`list[LintResult]`
- **触发条件**：文档包含 `4.B` 或 `CLI 接口` 相关标题
- **匹配模式（exit code 行）**：`\|\s*(\d+)\s*\|`
- **正例**：`| 0 | 成功 |` → 提取 code 0；`| 1 | 验证失败 |` → 提取 code 1；`| 2 | 使用错误 |` → 提取 code 2
- **反例**：`exit code 为 0` → 不匹配（非表格格式）；`| code | 含义 |` → 不匹配（表头）；`| --- | --- |` → 不匹配（分隔行）

#### PatternExampleChecker
- **职责**：当检测到章节 8（模式定义约束）时，验证每个检查器都有 3 正例 + 3 反例
- **输入**：`Document` 的章节 8 内容
- **输出**：`list[LintResult]`
- **触发条件**：文档包含章节 8
- **检查逻辑**：计算"正例"和"反例"区块内的编号列表项数量
...
正例（应匹配）:
  1. `数据库选型: TBD` → 英文标记
  2. `认证方式待定` → 中文标记
  3. `TODO: 补充错误码` → TODO 标记

反例（不应匹配）:
  1. `## TBDMarkerChecker` → 代码块内跳过
  2. `completed` → 无关词汇
  3. `table_definition` → 部分匹配但有词界

已知边界:
  - 代码块（```...```）内的内容不检查
  - 类名或变量名中包含 TBD 时在代码块外会误报
